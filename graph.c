...
int dfsIterative(Graph* graph, Vertex* source, Vertex* destination)
{
    ...
    while (dequeIsEmpty(stack) == 0) {
            
        currentVertex = dequeFront(stack);
        ...
        if(currentVertex == destination){
            ...
            for(int i = 0; i < currentVertex->numNeighbors; i++){
                
                if (currentVertex->neighbors[i]->isVisited == 0) {
                    dequePushBack(stack, currentVertex->neighbors[i]);
                }
            }
        }
...
int bfsIterative(Graph* graph, Vertex* source, Vertex* destination)
{
    ...
    while (!dequeIsEmpty(queue)) {
        ...
        for (int i = 0; i < currentVertex->numNeighbors; i++) {
            if (currentVertex->neighbors[i]->isVisited == 0) {
                dequePushBack(queue, currentVertex->neighbors[i]);
            }
        }
